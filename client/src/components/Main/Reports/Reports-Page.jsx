...
export default function ReportsPage({ pageLayout, currentUser }) {
    ...
    const deptChartData = {
        ...
        datasets: [{
            data: departmentBreakdown.map(d => parseInt(d.employee_count) || 0),
            backgroundColor: [
                'rgba(0, 0, 0, 0.9)',
                'rgba(0, 0, 0, 0.7)',
                'rgba(0, 0, 0, 0.5)',
    ...
    const downloadCSV = () => {
        const headers = ['Date', 'Department', 'Total Amount', 'Employees', 'Status']
        const rows = reports.map(r => [
            r.pay_date || '-',
            r.department_name || 'All',
                ...
                <div className="flex flex-col items-center justify-start h-9/10 w-full p-5 gap-5 overflow-y-scroll">
                    ...
                    <div className="flex flex-row items-center justify-between w-full">
                        <div className="flex flex-col items-start justify-start">
                            <h2 className="text-md font-medium">Reports & Analytics</h2>
                            <p className="font-sans text-sm text-[rgba(0,0,0,0.6)]">Comprehensive reporting and data analysis tools</p>
                        </div>
                                    ...
                                    {reports.length > 0 ? (
                                        reports.map((report, index) => (
                                            <tr key={index} className="border-b hover:bg-gray-50">
                                                ...
                                                <td className="p-3 text-sm">
                                                    {report.department_code || report.department_name || 'All Departments'}
                                                </td>
                                        ...
                                        <tbody>
                                            {periodDetails.map((detail, index) => (
                                                <tr key={index} className="border-b hover:bg-gray-50">
                                                    ...
                                                    <td className="p-2 text-sm">{detail.department_code || detail.department}</td>
                                                    ...
                                                    <td className="p-2 text-sm text-right">${parseFloat(detail.bonuses || 0).toLocaleString()}</td>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 w-full">
                        ...
                        <div className="bg-white rounded-2xl border border-[rgba(0,0,0,0.2)] p-5">
                            <p className="text-3xl font-semibold">{statistics.reportTypes}</p>
